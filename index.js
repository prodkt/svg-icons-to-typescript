...
function activate(context) {
    ...
    let disposable = vscode.commands.registerCommand('svgr.batchConvert', batchConvertSvgs);
 ...
 */
function isSvgToReactConversion(oldUri, newUri) {
 ...
 */
async function convertSvgToReact(oldUri, newUri) {
    try {
        const svgContent = await (0, promises_1.readFile)(oldUri.fsPath, "utf-8");
        ...
        await (0, promises_1.writeFile)(newUri.fsPath, reactComponent);
...
function handleConversionError(error) {
    if (error instanceof Error) {
        vscode.window.showErrorMessage(`Error converting SVG to React: ${error.message}`);
    }
    else {
        vscode.window.showErrorMessage("Unknown error when converting SVG to React");
    }
}
/**
...
async function processFolder(folderPath) {
    try {
        const entries = await (0, promises_1.readdir)(folderPath, { withFileTypes: true });
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                ...
                await convertSvgToReact(vscode.Uri.file(fullPath), vscode.Uri.file(newPath));
            }
        }
    }
    catch (error) {
